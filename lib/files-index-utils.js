(function (global) {
    ...
    function wordcloudFromIndex(index, options) {
        const normalizedOptions = typeof options === 'number' ? { limit: options } : (options || {});
        const fileFilter = typeof normalizedOptions.fileFilter === 'function' ? normalizedOptions.fileFilter : () => true;
        const files = (Array.isArray(index && index.files) ? index.files : []).filter(fileFilter); // SOFTM-publishing-condition-filter 2026-07-10: 워드클라우드도 게시 상태 조건으로 원본 파일을 거른 뒤 집계
        ...
        const terms = Array.from(counts.entries())
            ...
            .slice(0, normalizedOptions.limit || 240)
            .map(([term, count]) => ({
                term,
                count,

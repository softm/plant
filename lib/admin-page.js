(function (window, document) {
    ...
    let indexData = null;
    let grantData = { schema_version: 1, private_paths: [], public_paths: [] };
    let allPaths = [];
    ...
    function publicPathSet() {
        return new Set((grantData.public_paths || []).map(path => String(path)));
    }
    ...
    function renderRows() {
        ...
        setStatus(`${rows.length.toLocaleString()}개 표시 · grant ${granted.size.toLocaleString()}개 · 공개 예외 ${publicPathSet().size.toLocaleString()}개`); // SOFTM-publishing-public-exception 2026-07-10: 관리자 저장 시 public_paths 존재를 확인할 수 있게 표시
    }
    ...
    function addGrantPath(path) {
        ...
        paths.add(value);
        const publicPaths = publicPathSet();
        publicPaths.delete(value);
        grantData.private_paths = Array.from(paths).sort();
        grantData.public_paths = Array.from(publicPaths).sort(); // SOFTM-publishing-public-exception 2026-07-10: 같은 경로가 private/public에 동시에 저장되지 않게 정리
    }
    ...
    async function saveGrant() {
        const payload = JSON.stringify({
            schema_version: publicPathSet().size > 0 ? 2 : Number(grantData.schema_version || 1),
            private_paths: Array.from(privatePathSet()).sort(),
            public_paths: Array.from(publicPathSet()).sort()
        }, null, 2);
    ...
    async function loadAdminData() {
        ...
        grantData = await fetchJsonOrDefault('grant.json', { schema_version: 1, private_paths: [], public_paths: [] });
        if (!Array.isArray(grantData.public_paths)) {
            grantData.public_paths = [];
        } // SOFTM-publishing-public-exception 2026-07-10: 기존 grant.json을 읽을 때 공개 예외 배열 기본값 보강

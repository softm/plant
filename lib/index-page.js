(function (window, document) {
    ...
    function moveDirectorySelection(section, currentItem, delta) {
        const itemNodes = Array.from(section.querySelectorAll('.directory-item'));
        if (!itemNodes.length) {
            return;
        }
        const currentIndex = Math.max(0, itemNodes.findIndex(node => node._directoryItem === currentItem));
    ...
    function updateSelectedListHighlight(shouldScroll = false) {
        ...
        const listContainer = document.getElementById('file-list') || fileList.parentElement;
